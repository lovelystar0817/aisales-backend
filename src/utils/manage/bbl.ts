...
/**
 * Helper function to parse BBL feedback scores from session
 * BBL has advisoryTechnique and processAdherence
 */
export function parseBBLFeedbackScores(session: any) {
  try {
    const advisoryTechnique = session.roleplay?.feedback?.advisoryTechnique
    ...
    const advisoryTechniqueScore = advisoryTechnique?.overallScore ?? null;
    const processAdherenceScore = processAdherence?.overallScore ?? null;

    // Calculate overall score based on available scores
    const validScores = [advisoryTechniqueScore, processAdherenceScore].filter(
      (score) => score !== null,
    );
    ...
    return {
      advisoryTechniqueScore,
      processAdherenceScore,
      overallScore,
      isValid:
        advisoryTechniqueScore !== null && processAdherenceScore !== null,
    };
  } catch (error) {
    console.warn('Failed to parse BBL feedback scores for session:', {
      sessionId: session._id?.toString() || 'unknown',
      error: error instanceof Error ? error.message : String(error),
      advisoryTechniqueRaw: session.roleplay?.feedback?.advisoryTechnique,
      processAdherenceRaw: session.roleplay?.feedback?.processAdherence,
    });

    return {
      advisoryTechniqueScore: null,
      processAdherenceScore: null,
      productKnowledgeScore: null,
      overallScore: null,
      isValid: false,
      isPortfolioReview: false,
    };
  }
}
...
export async function getBBLStandingsSummary(
  companyId: string,
  dateFrom?: string,
  dateTo?: string,
  teams?: string[],
  ...
  for (const session of sessions) {
    const scores = parseBBLFeedbackScores(session);
...
export async function getBBLPracticeDetails(
  companyId: string,
  dateFrom?: string,
  dateTo?: string,
  teams?: string[],
  ...
  return generateBBLPracticeBreakdown(sessions, companyId, languageCode);
}

/**
 * Generate practice breakdown for BBL scoring system
 */
export function generateBBLPracticeBreakdown(
  sessions: any[],
  companyId?: string,
  languageCode: string = 'en',
...
export async function getBBLStandingsProgress(
  companyId: string,
  months: number = 6,
  module?: string,
  teams?: string[],
  ...
  return generateBBLProgressData(sessions, months, module);
}

/**
 * Generate progress data for BBL scoring system
 */
export function generateBBLProgressData(
  sessions: any[],
  months: number,
  module?: string,
): ProgressDataResult {
  ...
  for (const session of sessions) {
    ...
    const scores = parseBBLFeedbackScores(session);
    if (scores.advisoryTechniqueScore !== null) {
      monthlyData[monthKey].advisoryTechnique.push(
        scores.advisoryTechniqueScore,
      );
    }
  ...
  const advisoryTechniqueData = monthLabels.map((month) => {
    const scores = monthlyData[month].advisoryTechnique;
  ...
  const charts = [
    {
      name: 'Advisory Technique',
      data: advisoryTechniqueData,
      color: '#1C7AEB',
    },
    {
...
export async function getBBLUserStatistics(
  userId: string,
  companyId: string,
  module?: string,
) {
...
export async function getBBLUserPracticeSummary(
  userId: string,
  companyId: string,
) {
  const { sessionFilter } = await buildSessionFilterWithCompany({
...
export async function getBBLUserPracticeBreakdown(
  userId: string,
  companyId: string,
) {
  const { sessionFilter } = await buildSessionFilterWithCompany({
  ...
  return generateBBLPracticeBreakdown(sessions);
}

export async function getBBLUserProgressData(
  userId: string,
  companyId: string,
  months: number = 6,
  module?: string,
...
/**
 * Bulk version of getBBLUserStatistics to avoid N+1 queries
 */
export async function getBulkBBLUserStatistics(
  userIds: string[],
  companyId: string,
  module?: string,
  teams?: string[],
...
/**
 * Get session history for a BBL user
 */
export async function getBBLUserSessionHistory(
  userId: string,
  companyId: string,
  page: number = 1,
  limit: number = 10,
  ...
  const sessionHistory = sessions.map((session: any) => {
    ...
    // BBL uses advisoryTechnique and processAdherence
    const bblScores = parseBBLFeedbackScores(session);
    const score =
      bblScores.isValid && bblScores.overallScore !== null
        ? Math.round(bblScores.overallScore)
        : 0;
