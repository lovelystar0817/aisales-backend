...
export const ktAxaRecruitmentSoftSkillsSchema = z.object({
...
const ktAxaRecruitmentSoftSkillsPrompt = `You are an expert coach specializing in agent recruitment for Krungthai-AXA Life Insurance (KT-AXA). Your task is to evaluate **only the user's** soft skills performance during recruitment conversations.
...
{{
  "ktAxaSoftSkills": {{
    "description": "Measures soft skills demonstrated during KT-AXA agent recruitment conversations.",
    "overallScore": 76,
    "maxScore": 100,
    "sections": [
      {{
        ...
        "suggestion": "Simplify language: Instead of 'leverage your existing skill set,' say 'use the skills you already have.' Example: 'You mentioned managing your household budget—that's exactly the kind of planning skill we use in insurance advising.'"
      }},
      {{
        "title": "Relationship Building",
        "score": 20,
        ...
        "suggestion": "Continue building rapport by asking about their goals: 'What would success look like for you in this role? What are you hoping to achieve with KT-AXA?'"
      }},
      {{
        "title": "Adaptability",
        "score": 17,
        ...
        "suggestion": "Be more adaptive: When they mention time constraints, immediately pivot to flexibility. Example: 'I hear that time is a concern. Let me show you how flexible this role can be—many KT-AXA agents work around their existing schedule.'"
      }},
      {{
        "title": "Customer Orientation",
        "score": 20,
        "maxScore": 25,
        "why": "You presented KT-AXA well but could have been more convincing about the business opportunity and benefits.",
        "suggestion": "Strengthen your presentation: Be specific about benefits. Example: 'KT-AXA offers competitive commission structures with 30%% FYC, plus quarterly bonuses and the AXA Prime Blue training program. Many agents earn significant additional income while maintaining flexibility.'"
      }}
    ]
  }}
}}`;

export function getKtAxaRecruitmentSoftSkillsPrompt(
  characterName: string,
  languageCode: string = 'en',
): ChatPromptTemplate {
  let basePrompt = ktAxaRecruitmentSoftSkillsPrompt.replaceAll(
    '{{characterName}}',
    characterName,
  );
