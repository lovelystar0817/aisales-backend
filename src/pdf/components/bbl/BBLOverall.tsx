/**
 * BBL assessment overview component
...
interface BBLOverallProps extends BaseOverallProps {
  salesTechniqueScore?: number;
  productKnowledgeScore?: number;
}

const BBLOverall: React.FC<BBLOverallProps> = ({
  salesTechniqueScore,
  ...
  // BBL uses equal weighting for both scores
  const overallScore = (() => {
    if (salesTechniqueScore && productKnowledgeScore) {
      return Math.round((salesTechniqueScore + productKnowledgeScore) / 2);
    } else if (salesTechniqueScore) {
      return salesTechniqueScore;
    } else if (productKnowledgeScore) {
            ...
            >
              {salesTechniqueScore && (
                <ScoreIndicator
                  ...
                  label={translations.advisoryTechnique || 'Advisory Technique'}
                  translations={translations.ratings}
                />
              )}
              {productKnowledgeScore && (
